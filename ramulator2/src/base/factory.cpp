...
bool Factory::register_interface(std::string ifce_name, std::string ifce_desc) {
  ...
  if (auto it = m_registry.find(ifce_name); it == m_registry.end()) {
    ...
    // Interface already registered, just return true instead of throwing an error
    // This happens when the library is loaded multiple times (e.g., in Bluespec)
    DEBUG_LOG(DFACTORY, Logging::get("Base"), "Interface {} already registered, skipping...", ifce_name)
    return true;
  }
};
...
bool Factory::register_implementation(std::string ifce_name, std::string impl_name, std::string impl_desc, const Constructor_t& cstr) {
  ...
  if (auto ifce_it = m_registry.find(ifce_name); ifce_it != m_registry.end()) {
    ...
    if (auto impl_it = impls_info.find(impl_name); impl_it == impls_info.end()) {
      ...
      // Implementation already registered, just return true instead of throwing an error
      // This happens when the library is loaded multiple times (e.g., in Bluespec)
      DEBUG_LOG(DFACTORY, Logging::get("Base"), "Implementation {} of interface {} already registered, skipping...", impl_name, ifce_name)
      return true;
    }
